...
complete3 = complete3 %>%
  mutate(molc02= (101325*complete3$bucket)/(8.314*complete3$Kelvin))
```
...
time1 = subset(complete3, subset = complete3$Time ==0) %>%
  select(date.collection, Plot_ID,`treatment type`, bucket, grams_m2)
...
completeflux = left_join(timenew, time3, by = c("date.collection","bucket","treatment type", "Plot_ID")) %>%
  rename( "time1" = "grams_m2.x", "time2" = "grams_m2.y", "time3" = "grams_m2" )
```

```{r}
#ggplot

ggplot(data = completeflux, aes( x = date.collection, y = time3, color = `treatment type`)) + geom_point() +
  facet_wrap(.~ Plot_ID)
```
